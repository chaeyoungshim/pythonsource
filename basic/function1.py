...
# return, 튜플을 사용해서 리턴됨(콤마로 여러개 나열로 리턴할 수 있음 => 여러개를 리턴하는 것보다는 튜플로 만들어서 하나로 리턴하는 의미)
# def sum_and_mul(a, b):
#     return a + b, a * b
...
# print_n_items("안녕하세요")
# print()
# print_n_items("안녕하세요", 5)

# # 초기화시키고 싶은 입력 변수는 뒤쪽에 선언(man=True)
...
# say_myself("홍길동", 27)  # 기본값을 True 라고 잡았기 때문에 남자라고 나옴
...
# print("result = ", add_many(35,48,17,16,15))
# print("result = ", add_many())
...
# def var_test():
#     global a  # 전역변수인 a 사용할 것이라는 의미(함수 밖에 있는)
#     a = a + 1
...
# lotto 리스트에 6개의 숫자가 다 채워질 때까지 get_number() 를 호출하는 코드 작성
lotto = []

while True:
    num = get_number()

    if lotto.count(num) == 0:  # 중복 제거
        lotto.append(num)
